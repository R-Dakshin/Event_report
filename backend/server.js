...
const allowedOrigins = (process.env.ALLOWED_ORIGINS || 'http://localhost:3000,http://localhost:5173,http://localhost:5000').split(',').map((origin) => origin.trim()).filter(Boolean);
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', database: isMongo ? 'MongoDB' : 'Local JSON' });
});
...
const seedDatabase = async () => {
  try {
    ...
    // 2. Seed default clubs if database is incomplete
    const existingClubs = await db.clubs.find();
    if (existingClubs.length < 138) {
      console.log('Seeding initial VIT Chennai Clubs & Chapters (138 entries)...');
      // Clean up old seeds to prevent duplicates
      ...
      const defaultClubs = [
        ...
        { name: 'AUTO-VIT', description: 'Automotive Club' }, // AutoVIT fallback
      ...
      console.log('Seeded all 138 VIT Chennai clubs and chapters successfully.');
    }
  } catch (error) {
